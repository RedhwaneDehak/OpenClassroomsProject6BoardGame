...
var Game = {
    ...
    createWeapons: function () {
        ...
        honey = new Weapon("honey", "weapon", 30, "img/honey.png");
        fish = new Weapon("fish", "weapon", 40, "img/fish2.png");
        Game.placeItem(grass);
        Game.placeItem(berries);
        Game.placeItem(honey);
        Game.placeItem(fish);
    },

    // Creating player objects and placing players on a game board
    createPlayers: function () {
        player1 = new Player("White Bear", "player", 1, 100, grass);
        player2 = new Player("Dark Bear", "player", 2, 100, grass);
    ...
    changeWeapon: function () {
        ...
        switch (true) {
            // When he collects grass
            case weaponType.contains("grass"):
                if (activePlayer === "player1") {
                    var playerActualWeapon = player1.weapon.name;
                    player1.weapon = grass;
                } else {
                    var playerActualWeapon = player2.weapon.name;
                    player2.weapon = grass;
                }
                playerCell.classList.remove("grass");
                break;
                // When he collects berries
            case weaponType.contains("berries"):
                if (activePlayer === "player1") {
                    var playerActualWeapon = player1.weapon.name;
                    player1.weapon = berries;
                } else {
                    var playerActualWeapon = player2.weapon.name;
                    player2.weapon = berries;
                }
                playerCell.classList.remove("berries");
                break;
                // When he collects honey
            case weaponType.contains("honey"):
                if (activePlayer === "player1") {
                    var playerActualWeapon = player1.weapon.name;
                    player1.weapon = honey;
                } else {
                    var playerActualWeapon = player2.weapon.name;
                    player2.weapon = honey;
                }
                playerCell.classList.remove("honey");
                break;
                // When he collects fish
            case weaponType.contains("fish"):
                if (activePlayer === "player1") {
                    var playerActualWeapon = player1.weapon.name;
                    player1.weapon = fish;
                } else {
    ...
    player1FightTurn: function (event) {
        if (event.target !== event.currentTarget) {
            if (event.target.classList.contains("button-attack")) {
                ...
                player1Defend = true;
                player1FightMessageElem.text("You are defending against next attack");
            }
            player1FightButtonsElem.hide();
            player2FightButtonsElem.show();
            if (player2Defend) {
